...
class SymbolCacheService:
    ...
    async def get_symbol(self, query: str) -> str | None:
        ...
        result = await self.collection.find_one({"$or": [{"company_name": normalized}, {"aliases": normalized}]})
    ...
    async def cache_symbol(self, company_name: str, symbol: str):
        ...
        if existing:
            ...
            if normalized not in existing.get("aliases", []):
                await self.collection.update_one({"_id": existing["_id"]}, {"$addToSet": {"aliases": normalized}})
            return
